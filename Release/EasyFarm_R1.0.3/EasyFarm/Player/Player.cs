...
namespace EasyFarm.PlayerTools
{
    ...
    public class Player
    {
        ...
        /// <param name="unit"></param>
        /// <returns></returns>
        private bool IsTarget(Unit unit)
        {
        ...
        private void ExecuteActions(List<Ability> actions, Unit unit)
        {
            foreach (var act in actions)
            {
                ...
                {
                    int SleepDuration = act.IsSpell ? (int)Math.Ceiling(act.CastTime) * 1000 : 2000;
                    MaintainHeading(unit);
        ...
        /// </summary>
        /// <returns></returns>
        internal bool CanRest()
        {
            return (Session.Player.HPPCurrent < StandUPHPPValue && !HasAggro() && IsWorking &&
                Session.Player.HPCurrent > 0 && !IsDebuffPreventingResting(Session.Player.StatusEffects));
        }
    }
}